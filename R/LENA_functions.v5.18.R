                             ...
                             is.decreasing = TRUE,min.k = 3,max.k = 30,n_skip = 3)
{
  ...
  ptm <- proc.time()
  if (!is.matrix(bcnt)) bcnt = as.matrix(bcnt)
  no = t(bcnt) %*% bcnt
  ng.per.cell = colSums(bcnt,na.rm = T)
  ng = matrix(rep(ng.per.cell,ncol(bcnt)),nrow = ncol(bcnt))
  
  jac = no/(ng + t(ng) - no);rm(ng,no)
  
  jj.lst = apply(jac,1,function(x,mk) {
    qc = quantile(x,probs = (length(x) - 2*mk)/length(x))
    which(x >= qc)
  },mk = max.k)
  
  rm(jac);gc()
  cat("- runtime to subset cells:\n")
  print(proc.time() - ptm)
